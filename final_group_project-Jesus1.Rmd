...
df<-card_fraud_train %>% 
  select(category, amt, state, age, month_name, wday, hour, is_fraud, distance_miles)

#Just change data=df
#Prepare the recipe for the data
fraud_rec <- recipe(is_fraud ~ ., data = df) %>% #Load the dataset
  #Log normalize the miles to improve the indicator
  step_log(distance_miles, amt, hour, age) %>% 
  #Set others to the state variable
  step_other(state, threshold = .05) %>% 
  #Normalize numerical variables
  step_normalize(all_numeric(), -all_outcomes(), -month_name, -wday) %>% 
  #Format Nominal variables
  step_novel(all_nominal(), -all_outcomes(), month_name, wday) %>%
  #Create dummies for all the nominal variables
  step_dummy(all_nominal(), -all_outcomes(), month_name, wday) %>%
  #Remove variables with zero variance
  step_zv(all_numeric(), -all_outcomes()) %>%
  #Eliminate variables with high correlation across other regressor
