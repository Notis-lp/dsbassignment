...
fraud_rec <- recipe(is_fraud ~ ., data = card_fraud_train) %>%
  step_log(distance_miles, amt) %>%   #Log normalize the miles to improve the indicator
  step_other(state, threshold = .05) %>% #FROM EDA
...
cost <- cost %>%
  ...
  #fraud / amount / distance / age / hour / weekday / category / .... !jobs
...
```{r}
#Cost of using a naive approach
naive<-scales::dollar(cost_summary$false_naives)

false_negatives<-scales::dollar(cost_summary$false_positives)

false_positives<-scales::dollar(cost_summary$false_negatives)

loss_for_false_positives<-scales::dollar(cost_summary$false_positives * 0.02)
